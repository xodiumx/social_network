...
from .schemas import CreatePostSchema, GetPostSchema
...
async def get_all(
        ...
        service: PostService = Depends()) -> list[GetPostSchema]:
    """Get all posts."""
...
async def get_one(
        post_id: int,
        user: User = Depends(current_user),
        service: PostService = Depends()) -> GetPostSchema:
    """Get a single post."""
...
async def create(
        ...
        user: User = Depends(current_user),
        service: PostService = Depends()) -> CreatePostSchema:
    """Post creation."""
...
async def update(
        ...
        data: CreatePostSchema,
        user: User = Depends(current_user),
        service: PostService = Depends()) -> str:
    """Post update"""
